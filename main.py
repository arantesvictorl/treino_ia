...
Com base nesse template, forneça um plano de treino de musculação personalizado com as fichas (A,B,C ou mais dependendo do objetivo e grau) para o usuário, sendo cada ficha para um dia diferente. Considere as informações específicas inseridas nos campos do template para ajustar o treino de acordo com as necessidades e objetivos do usuário, garantindo um plano bem alinhado às suas características e metas de saúde e fitness. Lembre que a saída deve ser apenas as fichas de treino e nada mais. Nas fichas faça a divisão para cada dia de treino, (baseado na frequencia) entre exercicios de Costas, Peito, Quadriceps, Posterior, Panturrilha, Triceps, Biceps, Abdomen... tente equilibrar da melhor maneira possível esses grupamentos musculares nas fichas que você decidir. Não se esqueça de incluir o número de séries e repetições para cada exercício, além de variar o numero de series e repetições dependendo do objetivo. NÃO COLOQUE EXERCÍCIOS DE SUPERIORES COM INFERIORES NO MESMO DIA.
"""
TEMPERATURE = 0.2
MAX_TOKENS = 480
def main():
    ...
    if st.button('Gerar Treino'):
        if not idade or not altura or not peso or not frequencia or not sexo or not grau or not objetivo or not tem_restricao:
            ...
            llm = ChatGoogleGenerativeAI(temperature=TEMPERATURE, max_tokens=MAX_TOKENS, model="gemini-pro", google_api_key=f"{st.secrets['GOOGLE_API_KEY']}")
